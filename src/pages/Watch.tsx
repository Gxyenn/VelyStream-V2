...
const Watch = ({ onWatch }: WatchProps) => {
  ...
  const [downloadingQuality, setDownloadingQuality] = useState<string | null>(
    null
  );
  ...
  const handleServerChange = async (serverId: string) => {
    ...
    try {
      ...
      toast.error("Gagal memuat server streaming.", {
        description: "Silakan coba server atau kualitas lain.",
      });
    } finally {
  ...
  const handleDownload = async (serverId: string, quality: string) => {
    ...
    toast.info("Mempersiapkan unduhan...", {
      description: `Kualitas: ${quality}`,
    });
    try {
      const url = await api.getServerUrl(serverId);
      // The URL from the API is often a safelink. Opening it in a new tab
      // is the most reliable way for the user to get to the download.
      window.open(url, "_blank");
    } catch (error) {
      console.error("Failed to get download link:", error);
      toast.error("Gagal mendapatkan tautan unduhan.", {
        description:
          "Server mungkin sedang offline atau tautan tidak valid. Silakan coba lagi nanti.",
      });
    } finally {
  ...
  const selectedServerName = serversForSelectedQuality.find(
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-4">
          ...
          {animeDetail && (
            ...
            >
              <Link to={`/anime/${cleanSlug(animeDetail.slug)}`}>← Back </Link>
            </Button>
          )}
        </div>

        <div
          className="relative mb-4 w-full overflow-hidden rounded-xl border bg-black"
        ...
        <div className="flex gap-2">
          {episode.has_previous_episode && episode.previous_episode && (
            <Button
              variant="secondary"
              onClick={() =>
                navigate(`/watch/${episode.previous_episode!.slug}`)
              }
              className="gap-2"
            >
